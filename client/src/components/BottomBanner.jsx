    ...
    return (
        <div className='relative mt-24'>
            ...
            <div className='absolute inset-0 flex flex-col items-center md:items-end md:justify-center pt-1 md:pt-0 md:pr-64'>
                <div>
                  <h1 className='section-heading text-primary mb-6'>Why We Are the Best?</h1>
                       {features.map((feature, index)=>(
                    <div key={index} className='flex items-center gap-4 mt-2'>
                        <img src={feature.icon} alt={feature.title} className='md:w-10 w-9' />
                        <div>
                          <h3 className='text-[clamp(25px,2vw,30px)] font-display leading-[1.15] tracking-[-0.01em]'>{feature.title}</h3>
